#include <math.h>
#include <cmath>
#include <cstdlib> // Tada, you used "rand()" and "RAND_MAX" without this library. 
#include "TdS_Math.h"
...
int TdS_QuatricEquation(double *x, double coef4, double coef3, double coef2, double coef1, double coef0)
{
	...
	if (fabs(coef4) < 1e-10) // 3rd-polynomial or less
	{
		...
		// 3rd-polynomial
		res = SolveP3(x, coef2 / coef3, coef1 / coef3, coef0 / coef3);
		TdS_BubbleSort(x, res);
	}
	else
		// 4th-polynomial
		res = SolveP4(x, coef3 / coef4, coef2 / coef4, coef1 / coef4, coef0 / coef4);
